...
- `generate_methods_on_render` (bool, default `true`): Auto-generate methods draft on render.
- `methods_style` (str, default `full`): Methods style (`full` or `concise`).
- `methods_output` (str, default `auto_methods.md`): Methods filename written under `export/`.

## Outputs
- `export_job_spec.json`: JSON payload sent to the export engine.
- `auto_methods.md` (or custom `methods_output`): Publication-oriented methods draft from project history.
...
- Methods draft is generated by post-render hook `hooks.export_generate_methods:main`.

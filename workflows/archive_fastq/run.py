                ...
                if archive_size_bytes == 0:
                    if _has_excluded_files(entry, exclude_patterns):
                        if (target_root / entry.name).is_dir():
                            cleanup_only = True
                            cleanup_only_reason = 'already-cleaned/FASTQ-only'
                            issues.append(f"Including cleanup-only run with existing archive target: {entry.name}")
                        else:
                            issues.append(f"Skipping already-cleaned/FASTQ-only run without archive target: {entry.name}")
                            continue
...
def _run_archive_tui(candidates: list[RunCandidate], rules_path: Path) -> str:
    ...
    def _curses_main(stdscr) -> tuple[str, int]:
        ...
        show_project_id = any(c.project_id for c in candidates)
        project_values = [c.project_id or "-" for c in candidates] if show_project_id else []
        ...
        def _render(status: str = "") -> None:
            ...
            h, w = stdscr.getmaxyx()
            stdscr.erase()
            stdscr.addnstr(0, 0, f"archive_fastq | archive candidates | runs={len(candidates)}".ljust(max(0, w - 1)), w - 1, color_header)
            stdscr.addnstr(1, 0, "Only runs older than retention are shown. k: keep/unkeep  u: keep-until  s: save+continue  q: quit".ljust(max(0, w - 1)), w - 1, color_help)
            list_top = 4
            list_h = max(1, h - 6)
            run_col = _column_width(run_ids, "Run ID", min_width=18, max_width=40)
            owner_col = _column_width(owner_values, "Owner", min_width=5, max_width=12)
            project_col = _column_width(project_values, "Project ID", min_width=10, max_width=36) if show_project_id else 0
            set_by_col = _column_width(set_by_values, "Set By", min_width=6, max_width=12)
            if show_project_id:
                row_fmt = f"{{keep:<4}} {{archive:<7}} {{run:<{run_col}}} {{owner:<{owner_col}}} {{project:<{project_col}}} {{ku:<10}} {{set_by:<{set_by_col}}} {{total:>10}} {{arch_size:>10}}"
                header = row_fmt.format(keep="Keep", archive="Archive", run="Run ID", owner="Owner", project="Project ID", ku="Keep Until", set_by="Set By", total="Total", arch_size="ArchSize")
            else:
            ...
            for row in range(list_h):
                ...
                if show_project_id:
                    row_values['project'] = _clip(item.project_id or '-', project_col)
...
def _print_plan(
    ...
    show_project_id = any(c.project_id for c in candidates)
    project_col = max(10, min(36, max(len(c.project_id or '-') for c in candidates) + 2)) if show_project_id else 0
    status_col = 20
    total_col = 14
    arch_col = 24
    if show_project_id:
        row_fmt = (
            f"{{no:>4}}  {{run:<{run_col}}}{{owner:<{owner_col}}}{{project:<{project_col}}}"
    ...
    if show_project_id:
        header_values['project'] = 'Project ID'
    ...
    for idx, item in enumerate(candidates, start=1):
        ...
        if show_project_id:
            row_values['project'] = item.project_id or '-'
    ...
    if 'permission denied' in blob and str(rec.get('copy_status')) == 'failed':
        if '[sender]' in blob or 'send_files failed to open' in blob or (source_path and source_path in blob):
            return (
                ...
                'Fix ownership/permissions on the source run files so archive_fastq can read them.',
            )
        if '[receiver]' in blob or '[generator]' in blob or (target_path and target_path in blob):
            return (
                'archive target permission denied',
                'Fix ownership/permissions under the target archive run directory and rerun archive_fastq.',
            )
        return (
...
def main() -> None:
    ...
    try:
        ...
        _append_log(log_path, f"archive_fastq start: runs={len(selected_candidates)} total_size={required}")
        ...
        _append_log(
            log_path,
            f"archive_fastq done: processed={len(records)} copy_failures={copy_failures} verify_failures={verify_failures} cleanup_done={cleanup_done} cleanup_failures={cleanup_failures}",
        )
